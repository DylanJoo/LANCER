...
# multiquert retrieval
retrievers = ["qwen3-mds-duc04-mq-2ps+sq"]
...
def search_mds(query, searcher, service_name="bm25-mds-duc04", limit=100, **kwargs):
    ...
    if 'qwen3' in service_name:
        raise NotImplementedError("Use pre-retrieved result instead.")
...
for retriever in retrievers:
    ...
    if args.overwrite:
        for qid in queries:
            ...
            result = dict(sorted(result.items(), key=lambda item: item[1], reverse=True))  # sort by score
        run[qid] = result
        ...
        with open(f"{retriever}.run", 'r') as f:
            for line in f:
                ...
                run[qid_][docid] = float(score)

    reranker = 'none'

    for qid in queries:
        ## reconnect to rac_data and evaluate
        if 'mq' in retriever and 'qwen3' not in retriever:
            with open(f'/home/hltcoe/jhueiju/temp/mds-duc04/{qid}_subtopics_{args.n_subquestions}.json', 'r') as f:
            ...
            for subq in subquestions:
                ...
                print(qid, subq)
                result = search_mds(subq, searcher=searcher, service_name=retriever, limit=100)
            ...
            run = {qid: [docid for docid, _ in final_ranking]}
        ...
        rac_data[qid] = {  # TODO: might be missing some fields
            ...
            "docids": [docid for docid in run[qid]],
